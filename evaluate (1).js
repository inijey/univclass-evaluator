...
// 등급/이유 누락 방어 로직: AI 응답에 빈 값이 있으면 눈에 띄는 기본값으로 채워
// "정상적으로 채워진 것처럼" 착각하는 일이 없도록 함
function sanitizeEvalResult(parsed) {
  const keys = ['응답속도정확성', '주도적안내격려', '피드백구체성', '정서적지지동기'];
  keys.forEach(k => {
    if (!parsed[k] || typeof parsed[k] !== 'object') {
      parsed[k] = {};
    }
    if (!['상', '중', '하'].includes(parsed[k].등급)) {
      parsed[k].등급 = '중';
    }
    const reason = (parsed[k].이유 || '').trim();
    if (!reason) {
      parsed[k].이유 = '⚠️ AI가 근거를 생성하지 못했습니다. 원본 로그를 확인해 직접 입력해주세요.';
    }
  });
  return parsed;
}
...
중요:
1. 반드시 모든 4개 영역에 등급(상/중/하 중 하나)을 빠짐없이 입력하세요. 등급이 누락되면 안 됩니다.
2. 모든 영역의 "이유"는 절대 빈 문자열이나 공백일 수 없습니다. 반드시 대화 로그 속 실제 날짜·행동을 근거로 한 구체적인 문장을 1~2개 작성하세요.
3. 대화 내용이 부족해 특정 영역을 판단하기 어렵더라도, "이유" 칸에는 왜 판단이 어려운지(예: 해당 영역과 관련된 대화가 로그에 거의 없어 하로 평가함)를 반드시 서술하세요. 절대 이유를 비워두지 마세요.`;
  ...
  if (allMonths && isCSV) {
    ...
    for (const month of monthsToEval) {
      ...
      try {
        ...
        const monthParsed = sanitizeEvalResult(JSON.parse(raw));
        monthResults[month] = monthParsed;
      } catch (e) {
        continue;
      }
    }
  ...
  try {
    ...
    const data = await response.json();
    const raw = data.content[0].text.trim().replace(/```json|```/g, '').trim();
    const parsed = sanitizeEvalResult(JSON.parse(raw));
